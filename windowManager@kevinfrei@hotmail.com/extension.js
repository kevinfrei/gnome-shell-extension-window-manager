...
let ArrangeMenu = GObject.registerClass(class ArrangeMenu extends PanelMenu.Button {
    _init() {
        ...
        this.add_actor(icon);
        this.menu.addAction(_('Cascade'), () => this.cascadeWindow(), this._getCustIcon('cascade-windows-symbolic'));
        this.menu.addAction(_('Tile'), () => this.tileWindow(), this._getCustIcon('tile-windows-symbolic'));
        this.menu.addAction(_('Side by side'), () => this.sideBySideWindow(), this._getCustIcon('sidebyside-windows-symbolic'));
        this.menu.addAction(_('Stack'), () => this.stackWindow(), this._getCustIcon('stack-windows-symbolic'));
        this.menu.addAction(_('Maximize'), () => this.maximizeWindow(Meta.MaximizeFlags.BOTH), this._getCustIcon('maximize-windows-symbolic'));
        this.menu.addAction(_('Maximize Vertical'), () => this.maximizeWindow(Meta.MaximizeFlags.VERTICAL), this._getCustIcon('maximize-vertical-windows-symbolic'));
        this.menu.addAction(_('Maximize Horizontal'), () => this.maximizeWindow(Meta.MaximizeFlags.HORIZONTAL), this._getCustIcon('maximize-horizontal-windows-symbolic'));
        this.menu.addAction(_('Restoring'), () => this.restoringWindow(), this._getCustIcon('restoring-window-symbolic'));
    ...
    tileWindow() {
        ...
        for (let windowI = 0; windowI < windows.length; windowI++) {
            ...
            for (let cellJ = 0; cellJ < gridCells.length; cellJ++) {
                ...
                const dist = Math.sqrt(Math.pow((windowCenterX - cell.centerX), 2) +
                    Math.pow((windowCenterY - cell.centerY), 2));
        ...
        for (let i = 0; i < windows.length; i++) {
            ...
            let minI, minJ;
            windowIsToMove.forEach((windowI) => cellJsToFill.forEach((cellJ) => {
        ...
        for (let i = 0; i < windows.length; i++) {
            ...
            switch (direction) {
                ...
                case Meta.MaximizeFlags.VERTICAL:
                    win.move_resize_frame(true, win.get_frame_rect().x, workArea.y, win.get_frame_rect().width, workArea.height);
                    break;
                case Meta.MaximizeFlags.HORIZONTAL:
                    win.move_resize_frame(true, workArea.x, win.get_frame_rect().y, workArea.width, win.get_frame_rect().height);
                    break;
            }
        }
    }
...
let Column = GObject.registerClass(class Column extends PanelMenu.SystemIndicator {
    _init() {
        super._init();
...
function addKeybinding() {
    ...
    Main.wm.addKeybinding(HOTKEY_CASCADE, arrange._gsettings, Meta.KeyBindingFlags.NONE, modeType, arrange.cascadeWindow.bind(arrange));
    Main.wm.addKeybinding(HOTKEY_TILE, arrange._gsettings, Meta.KeyBindingFlags.NONE, modeType, arrange.tileWindow.bind(arrange));
    Main.wm.addKeybinding(HOTKEY_SIDEBYSIDE, arrange._gsettings, Meta.KeyBindingFlags.NONE, modeType, arrange.sideBySideWindow.bind(arrange));
    Main.wm.addKeybinding(HOTKEY_STACK, arrange._gsettings, Meta.KeyBindingFlags.NONE, modeType, arrange.stackWindow.bind(arrange));
}
